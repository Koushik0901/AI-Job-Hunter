...
from dashboard.backend.resume_agents_swarm.prompts import (
    load_json_repair_prompt,
    load_resume_evidence_miner_prompt,
    load_resume_jd_decomposer_prompt,
    load_resume_planner_prompt,
    load_resume_rewriter_prompt,
    load_resume_scoring_prompt,
)
...
def load_llm_config() -> LLMConfig:
    ...
    return LLMConfig(
        api_key=api_key,
        scoring_model=os.getenv("RESUME_SWARM_SCORING_MODEL", "openai/gpt-oss-120b").strip() or "openai/gpt-oss-120b",
        rewrite_model=os.getenv("RESUME_SWARM_REWRITE_MODEL", "openai/gpt-oss-120b").strip() or "openai/gpt-oss-120b",
        scoring_temperature=float(os.getenv("RESUME_SWARM_SCORING_TEMPERATURE", "0.05")),
        rewrite_temperature=float(os.getenv("RESUME_SWARM_REWRITE_TEMPERATURE", "0.2")),
        scoring_max_tokens=_int_or_none(os.getenv("RESUME_SWARM_SCORING_MAX_TOKENS")),
        rewrite_max_tokens=_int_or_none(os.getenv("RESUME_SWARM_REWRITE_MAX_TOKENS")),
    )
...
def build_scorer_messages(
    *,
    ...
    prompt = load_resume_scoring_prompt()
...
def build_jd_decomposer_messages(*, job_description: str) -> list[object]:
    prompt = load_resume_jd_decomposer_prompt()
    user_content = (
        "<JobDescription>\n"
        f"{job_description}\n"
        "</JobDescription>"
...
def build_evidence_miner_messages(
    *,
    ...
    prompt = load_resume_evidence_miner_prompt()
    user_content = (
        "<JDTargetSpecJSON>\n"
...
def build_planner_messages(
    *,
    ...
    prompt = load_resume_planner_prompt()
    user_content = (
        "<JDTargetSpecJSON>\n"
        f"{jd_target_spec_json}\n"
        "</JDTargetSpecJSON>\n\n"
...
def build_rewriter_messages(
    *,
    ...
    prompt = load_resume_rewriter_prompt()
    numbered = "\n".join(f"{idx:04d}: {line}" for idx, line in enumerate(current_latex_resume.splitlines()))
    evidence_json = json.dumps(evidence_context or {}, ensure_ascii=False)
    cards_json = json.dumps(project_cards or [], ensure_ascii=False)
    blocked = json.dumps(do_not_claim or [], ensure_ascii=False)
    pack_json = json.dumps(evidence_pack or {}, ensure_ascii=False)
    user_content = (
        ...
        "</RecruiterFeedbackJSON>\n\n"
        "<ResumeLatex>\n"
        f"{current_latex_resume}\n"
        "</ResumeLatex>\n\n"
        "<ResumeLatexLineCandidates>\n"
        f"{numbered}\n"
        "</ResumeLatexLineCandidates>\n\n"
        "<EvidenceContext>\n"
        f"{evidence_json}\n"
