...
from dashboard.backend.resume_agents_swarm.graph import build_resume_agents_swarm_graph
from dashboard.backend.resume_agents_swarm.llm import load_llm_config
from dashboard.backend.swarm_runtime import latex_to_text, score_from_history
...
def run_resume_agents_swarm_optimization(
    job_description: str,
    resume_text: str,
    latex_resume: str,
    ...
    graph = build_resume_agents_swarm_graph(config, on_event=progress_callback, should_cancel=should_cancel)

    resolved_resume_text = resume_text.strip() or latex_to_text(latex_resume)
    ...
    initial_state = {
        ...
        "initial_line_count": max(1, len(latex_resume.splitlines())),
    ...
    final_latex = str(final_state.get("latex_resume", latex_resume))
    ...
    if compile_guard_enabled:
        before_ok = _compile_ok("resume-before", latex_resume)
        after_ok = _compile_ok("resume-final", final_latex)
        if before_ok is True and after_ok is False:
            ...
            final_latex = latex_resume
            final_score = restored_score

    final_payload = {
        "final_latex_resume": final_latex,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="swarm-like resume optimization loop (score/rewrite/apply)")
    parser.add_argument("--jd", required=True, help="Job description text or path")
    parser.add_argument("--latex", required=True, help="LaTeX resume path or raw text")
...
def main() -> None:
    ...
    latex_resume = _read_path_or_text(args.latex)
