...
from ai_job_hunter.match_score import normalize_skill
    ...
    *,
    ...
    if recommendation == "hold":
        ...
        if fit_score >= 60 and urgency_score < 50:
            hold_reasons.insert(
                0, "The fit is acceptable, but the posting no longer looks urgent."
            )
        return {
            "guidance_mode": "evaluation",
    ...
    if friction_score >= 50:
        archive_reasons.insert(
            0, "Application friction is high enough to make this a poor use of time."
        )
    elif confidence_score < 45:
        archive_reasons.insert(
            0, "The model confidence is too low to justify active attention."
        )
    return {
        "guidance_mode": "evaluation",
...
def _stage_guidance(
    *,
    ...
    if normalized_status == "applied":
        ...
        if fit_score >= 75:
            stage_reasons.append(
                "Your profile overlap is still strong enough to justify keeping the role warm."
            )
        elif fit_score < 55:
            stage_reasons.append(
                "Use the response window to decide whether this is still worth proactive follow-up."
            )
        if urgency_score >= 70:
            stage_reasons.append(
                "The posting is still fresh enough to justify a timely follow-up."
            )
        elif urgency_score < 45:
            stage_reasons.append(
                "The posting is aging, so the next touchpoint matters more."
            )
    ...
    if normalized_status == "interviewing":
        ...
        if fit_score >= 75:
            stage_reasons.append(
                "Use your strongest evidence and matched skills as the center of interview prep."
            )
        elif fit_score < 55:
            stage_reasons.append(
                "Use the drawer as a prep checklist for the areas that still need tighter examples."
            )
        if friction_score >= 50:
            stage_reasons.append(
                "There are still some friction points worth preparing for."
            )
        confidence_reason = _confidence_reason()
        if confidence_reason:
            stage_reasons.append(confidence_reason)
    ...
    if normalized_status == "offer":
        ...
        if fit_score >= 75:
            stage_reasons.append(
                "The original fit looked strong enough to make the offer worth comparing seriously."
            )
        elif fit_score < 55:
            stage_reasons.append(
                "The fit was only moderate, so focus on scope and compensation trade-offs."
            )
        if friction_score >= 50:
            stage_reasons.append(
                "There may still be friction points to weigh before accepting."
            )
        confidence_reason = _confidence_reason()
        if confidence_reason:
            stage_reasons.append(confidence_reason)
...
def build_recommendation(
    *,
    ...
    raw_fit = int(job.get("raw_score") or job.get("fit_score") or match_score or 0)
    fit_score = clamp(raw_fit)
    ...
    likelihood = (
        ...
        + ((source_quality_score - 50) * 0.08)
        + ((role_quality_score - 50) * 0.06)
        - (friction_score * 0.28)
    )
    ...
    elif interview_likelihood_score >= 74:
        recommendation = "apply_now"
    elif interview_likelihood_score >= 56:
        recommendation = "review_manually"
    elif interview_likelihood_score >= 38:
        recommendation = "hold"
    else:
        recommendation = "archive"
    ...
    if override:
        ...
        if fit_score >= 76:
            reasons.append("Content fit is strong against your current profile.")
        elif fit_score >= 58:
            reasons.append("Content fit is workable but needs a quick manual read.")
        else:
            reasons.append("Content fit is weak relative to your current profile.")
        if age_days <= 3:
        ...
        if source_quality_score >= 66:
            reasons.append("This source has converted slightly better than your baseline.")
        elif source_quality_score <= 34:
            reasons.append("This source has underperformed in your history.")
        if friction_score >= 50:
            reasons.append(
                "Application friction is elevated because of missing evidence or role constraints."
            )
        elif red_flags:
...
def build_application_brief(
    *, profile: dict[str, Any], job: dict[str, Any]
    ...
    preferred = [
        str(item).strip()
        for item in enrichment.get("preferred_skills", [])
        if str(item).strip()
    ]
    ...
    emphasis: list[str] = []
    role_family = str(
        enrichment.get("role_family") or job.get("title") or "this role"
    ).strip()
    if profile.get("years_experience"):
        emphasis.append(
            f"Lead with your {int(profile['years_experience'])}+ years of experience that map to {role_family}."
        )
    ...
    if job.get("desired_title_match"):
        emphasis.append(
            "Make the title alignment explicit in your summary and top bullets."
        )
    if not emphasis:
        emphasis.append(
            "Lead with the closest relevant project or accomplishment before generic skills."
        )
    while len(emphasis) < 3:
        emphasis.append(
            "Use quantified impact and execution ownership rather than tool-only bullets."
        )
    ...
    return {
        ...
        + [item for item in matched_preferred if item not in matched_required],
